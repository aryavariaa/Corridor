...
|---|---|---|---|---|
| `Corridor Viewed` | Comparison results load for a corridor | Client (unchanged) | `corridor_id`, `send_country`, `receive_country`, `send_currency`, `receive_currency`, `tier` | Top-of-funnel: what people are actually comparing. `tier` is `Everyday`, `Large`, or `Custom` (a user-entered amount, debounced, so one event per settled amount, not per keystroke). |
